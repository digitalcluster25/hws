...
const lbl = { fontSize:'13.4px', fontWeight:400, letterSpacing:'0.12em', textTransform:'uppercase', color:C.mid }
const projects = [
  ...
  return (
    <div className="font-sans min-h-screen" style={{ color:C.dark }}>
      <section className="pt-40 pb-24" style={{ background:C.light, ...sec() }}>
        <div style={wrap}>
          <Link to="/" className="inline-flex items-center gap-2 mb-12 text-sm hover:opacity-60 transition-opacity" style={{ color:C.muted, textDecoration:'none' }}>← Главная</Link>
          <p style={{ ...lbl, color:C.muted }} className="mb-6">Портфолио</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-gray-200 border border-gray-200">
            {[{num:'300+',lb:'Проектов'},{num:'17+',lb:'Лет опыта'},{num:'4',lb:'Страны'},{num:'5★',lb:'Только luxury'}].map(({num,lb})=>(
              <div key={num} className="flex flex-col items-center gap-1 py-10 px-4">
                ...
                <span style={{ fontSize:'13px', color:C.mid, letterSpacing:'0.08em', textTransform:'uppercase' }}>{lb}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {projects.map((p)=>(
              <motion.div key={p.title} variants={item} className="flex flex-col" style={{ border:'1px solid #e5e7e0' }}>
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <button className="text-sm font-medium text-left hover:opacity-60 transition-opacity" style={{ color:C.dark, background:'none', border:'none', cursor:'pointer', padding:0 }}>Подробнее →</button>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      <section className="py-28 text-center" style={{ background:C.dark, ...sec() }}>
        <div style={wrap}>
          ...
          <a href="/#contact" className="inline-flex items-center justify-center font-semibold hover:opacity-80 transition-opacity"
            style={{ height:'2.75rem', padding:'0 2rem', borderRadius:'24px', background:'#fff', color:C.dark, fontSize:'0.95rem', textDecoration:'none' }}>Записаться на консультацию</a>
        </div>
      </section>
    </div>
  )
